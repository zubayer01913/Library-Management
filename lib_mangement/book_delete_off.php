...
if ((isset($_GET['ref_number'])) && ($_GET['ref_number'] != "")) {
  $deleteSQL = sprintf("DELETE FROM books WHERE ref_number=%s",
                       GetSQLValueString($_GET['ref_number'], "text"));

  mysql_select_db($database_lib_ss, $lib_ss);
  $Result1 = mysql_query($deleteSQL, $lib_ss) or die(mysql_error());

  $deleteGoTo = "book_index_off.php";
